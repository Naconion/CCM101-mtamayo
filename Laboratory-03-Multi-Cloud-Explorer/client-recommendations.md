...
- AWS Lambda – Runs application code without requiring the startup to manage servers and can automatically scale based on demand.
- Amazon DynamoDB – A managed NoSQL database suitable for applications that require fast and scalable data storage.
- Amazon S3 – Provides scalable object storage for images, videos, application files, backups, and other data.
...
- Azure Virtual Machines – Allows the university to migrate and run Windows Server workloads in the cloud.
- Microsoft Entra ID – Provides cloud-based identity and access management and integrates with existing Microsoft identity environments.
- Azure SQL Database – Provides a fully managed relational database service for university applications and systems.
- Azure Blob Storage – Provides scalable storage for documents, research data, backups, and other university files.
...
- Compute Engine – Provides configurable virtual machines that can be used for high-performance computing and demanding AI/ML workloads.
- Google Kubernetes Engine (GKE) – Provides a managed Kubernetes environment for running and scaling containerized AI and ML applications.
- Vertex AI – Provides an integrated platform for developing, training, deploying, and managing machine learning models.
- Cloud Storage – Provides scalable storage for large datasets, training data, models, and research files.
...
- Amazon EC2 – Provides scalable computing resources for hosting the company's e-commerce applications.
- Elastic Load Balancing (ELB) – Distributes incoming traffic across multiple application servers to improve availability and performance.
- Amazon EC2 Auto Scaling – Automatically increases or decreases the number of EC2 instances according to application demand.
